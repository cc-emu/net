...
bool RakPeer::Startup( unsigned short maxConnections, int _threadSleepTimer, SocketDescriptor *socketDescriptors, unsigned socketDescriptorCount, int threadPriority )
{
	...
	for (i=0; i<socketDescriptorCount; i++)
	{
		...
		if (socketDescriptors[i].remotePortRakNetWasStartedOn_PS3==0)
			rns->s = (uintptr_t) SocketLayer::Instance()->CreateBoundSocket( socketDescriptors[i].port, true, socketDescriptors[i].hostAddress, 100 );
		else // if (socketDescriptors[i].socketType==SocketDescriptor::PS3_LOBBY_UDP)
			rns->s = (uintptr_t) SocketLayer::Instance()->CreateBoundSocket_PS3Lobby( socketDescriptors[i].port, true, socketDescriptors[i].hostAddress );
				...
				else
				{
					...
					if (socketListIndex==socketList.Size())
					{
						...
						if (incomingRakNetSocket->remotePortRakNetWasStartedOn_PS3==0)
							rns->s = (uintptr_t) SocketLayer::Instance()->CreateBoundSocket( bindingAddress.port, true, ipList[foundIndex], 0 );
						else
							rns->s = (uintptr_t) SocketLayer::Instance()->CreateBoundSocket_PS3Lobby( bindingAddress.port, true, ipList[foundIndex] );
						if ((SOCKET)rns->s==(SOCKET)-1)
						{
